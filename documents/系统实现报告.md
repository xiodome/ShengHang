...
在评论互动模块中，评论存在层级关系（一条主评论可以有多条子回复，子回复可能还有回复）。删除评论时，需要删除它的所有子回复以防止数据孤岛，同时需要删除该评论对应的操作记录。为了保证这一删除约束，引入**触发器**：当删除评论时，递归地删除它的所有子评论（回复）。

```sql
-- 触发器：递归删除子评论（回复）
CREATE TRIGGER delete_comment_reply BEFORE DELETE ON Comment
    FOR EACH ROW
    BEGIN
        -- 当评论被删除前，自动删除 parent_id 为该评论ID 的所有子评论
        -- 数据库会因此再次触发本触发器，从而实现递归删除
        DELETE FROM Comment WHERE parent_id = OLD.commentId;
    END;
```

##### 2.1.2 自动更新触发器

在 record_play（记录播放功能）中，需要做两件事：

- 新增播放记录：INSERT INTO PlayHistory ... 
- 修改歌曲列表里的播放计数：UPDATE Song SET play_count = play_count + 1 

这两件事可以用触发器合并实现：每当 PlayHistory 表增加一条记录时，数据库自动去更新 Song 表
...
我们的代码在歌单、评论、专辑表记录了like_count项表示其被其他用户添加“喜欢”的次数。在新增和删除Favorite时，我们都要判断Favorite类型并在对应主表内更新数据。因此我们可以在 Favorite 表上建触发器，自动识别类型并更新对应主表的计数。
...
CREATE TRIGGER after_favorite_insert
    ...
    FOR EACH ROW
    BEGIN
        IF NEW.target_type = 'song' THEN
            UPDATE Song SET like_count = like_count + 1 WHERE song_id = NEW.target_id;
        ELSEIF NEW.target_type = 'album' THEN
            UPDATE Album SET like_count = like_count + 1 WHERE album_id = NEW.target_id;
        ELSEIF NEW.target_type = 'songlist' THEN
            UPDATE Songlist SET like_count = like_count + 1 WHERE songlist_id = NEW.target_id;
        END IF;
    END;
```
...
| visibility | string | 账户可见性   | 仅有三种值：私密、仅关注者可见、所有人可见 |
...
在这次数据库大作业中，我从零开始接触数据库设计和实现的过程。回顾整个过程，我不仅在技术能力上获得了显著提升，更在团队协作和项目管理上积累了宝贵的经验。

我们首先遇到困难是抽象我们的需求。由于我们刚开始经验都不是很足，所以我们不清楚需求的工作量和难度，我们刚开始还做不到在接触项目前期就完成完美的需求分析。我们的做法是先分析我们最基础的需求，然后在构建项目框架的同时从用户和开发者的角度深入思考项目，在中期不断迭代我们的需求。很高兴最终也是完成了一个模块化清晰，功能完善的项目。

实验和理论课是同步开展的。在实验初期，我们按照理论课的方法设计数据库。跟随理论课的知识学习，我们设计了从最初的简单ER图和数据流图设计到后期的存储过程和复杂的查询实现，逐步掌握了数据库设计的核心概念。

我参与了后端结构的设计和开发。在这个过程中，我后端业务逻辑编写的能力得到了大幅提高，也对SQL语言更加熟悉了，更重要的是培养了独立思考、解决实际问题的能力。

数据库的实验课不仅考验我们的数据库设计能力和编程能力，也对我们团队合作能力的提出要求。从最初的需求分析到后期的功能实现和优化的整个过程中，我们小组成员充分发挥各自的优势，分工协作，互相支持。在一人遇到困难时，我们都会通过网络进行联系，及时解决问题。 随着项目的进行，我们的协作能力得到了极大的提升。我们不仅在技术上互相帮助，还在项目管理上相互配合，合理分配任务，确保了项目的顺利进行。

总而言之，这个学期的数据库课程对我来说是一次难忘的经历。感谢老师和助教们的辛勤付出、无私的帮助和支持，感谢我的队友一路上的包容和支持。未来无论是否从事数据库、前后端的工作，这段学习经历所赋予的知识与技能都将伴随我前行，成为宝贵的财富。

### 欧阳晓宇感想

作为我个人第一次开展项目设计，同时也是第一次接触数据库相关设计，这次经历对我来说既充满挑战，也收获颇丰。在本次大作业中，我和我的组员从系统设计的初步构想到具体实现，一步步搭建了完整的音乐播放与管理平台——“声航音乐服务系统”。整个过程不仅让我对数据库设计有了更深刻的理解，也提升了我在前后端开发、系统集成及团队协作方面的能力。

在项目开发的前期我着重与系统设计上，在这个部分我遇到了许多的困难，但是根据实验指导的要求我一步步完成了整个系统的设计：从最开始的需求分析，从实际需求出发设计了多个模块，并对每个模块的功能进行了详细的设计，然后通过绘制数据流图进一步在全局与局部上把我系统的要求与运行机制；然后到数据库的概念模式的设计，这个部分为数据库绘制了对应的E-R图，为整个系统的实现提升到了概念层面；再之后到数据库的逻辑模式的设计，在这个阶段详细设计了数据库各个实体与实体之间的联系，并对数据库的关系模式进行了规范化以及优化，保证系统的规范性以及性能；最后就是完成数据库的物理设计，选取了对应的存储结构与存储方法，同时定义了相关索引。至此，我们系统的整体设计就已经跃然纸上了，只需要完成后续的系统实现即可。

在系统实现阶段，我主要负责了部分后端相应模块的设计与开发，这个阶段的工作让我对系统有了更深一步的了解，同时也极大增强了我的代码编写能力以及SQL语句的熟悉程度，让我独立完成相应需求的能力大大提高。同时我还负责了相应的前端的开发，在开发过程中我借助大模型的帮助，设计出了系统对应的前端界面，这个部分的工作让我更懂得如何使用大模型，通过大模型的帮助完成自己不熟悉领域上的工作，同时这个部分的工作也让我对前端的相关知识有了一定的了解，有利于我后续对于前端的学习。

总的来说，本次“声航音乐服务系统”的设计与实现，是一次难得的学习经历和成长机会。我深深感谢组员以及提供帮助的助教，让我能够顺利完成这一数据库实验项目，并从中收获了宝贵的实践经验。相信本次项目中的所学知识和经验将被运用到更复杂的系统开发中，并提升自己的专业能力。
