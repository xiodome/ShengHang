...
以上带*符号的部分是 需要实现时才能搞清楚的部分 或者 目前无法完成的部分

特别是关于管理员这部部分的内容

系统自动维护的工作是否也属于管理员的部分（？）

管理员实现：一个无法注册的超级用户账号（存在用户表中）（管理员有且只有一个）
...
保证专辑内的每一首歌歌手均包含该发行该专辑的歌手

规定歌手的第一张专辑发布时间为出道时间（若无专辑则为NULL）



一条评论可以有多条回复

但是保证评论与回复层数只有一层，即不能对回复继续回复（当然后面实现时可以考虑多层回复）

展示评论时评论状态可以为：'举报中'或者'正常'



对于播放记录，只有满足一定条件（如实际播放时间至少为歌曲的30%等）才计入一次播放记录
...
以上全部内容均不为最终内容，一切内容都可能会随着具体实现时修改
